    ...
    if (empty($_SESSION['hak_akses'])){
        ...
        <!-- Container START -->
            <div id="colres">
                ...
                echo '
                    <table class="bordered container-fluid" id="tbl">
                        <tbody>
                            <tr>
            ...
            <div id="lead">
                <h5>CAMAT LARANGAN</h5>
                <div style="height: 50px;"></div>';
                $query = mysqli_query($config, "SELECT camat, nip FROM tbl_instansi");
                list($camat,$nip) = mysqli_fetch_array($query);
                if(!empty($camat)){
                    echo '<p class="lead">'.$camat.'</p>';
                } else {
                    echo '<p class="lead"> </p>';
                }
                if(!empty($nip)){
