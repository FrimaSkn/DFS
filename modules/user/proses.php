...
// jika user sudah login, maka jalankan perintah untuk insert dan update
	...
	if ($_GET['act']=='insert') {
		if (isset($_POST['simpan'])) {
			...
			$password  = md5(mysqli_real_escape_string($config, trim($_POST['password'])));
			$nama_user = mysqli_real_escape_string($config, trim($_POST['nama_user']));
			$hak_akses = mysqli_real_escape_string($config, trim($_POST['hak_akses']));

			// perintah query untuk menyimpan data ke tabel users
            $query = mysqli_query($config, "INSERT INTO tbl_users(username,password,nama_user,hak_akses)
                                            VALUES('$username','$password','$nama_user','$hak_akses')")
                                            or die('Ada kesalahan pada query insert : '.mysqli_error($config));    
	...
	elseif ($_GET['act']=='update') {
		if (isset($_POST['simpan'])) {
			if (isset($_POST['id_user'])) {
				// ambil data hasil submit dari form
				$id_user            = mysqli_real_escape_string($config, trim($_POST['id_user']));
				$username           = mysqli_real_escape_string($config, trim($_POST['username']));
				$password           = md5(mysqli_real_escape_string($config, trim($_POST['password'])));
				$nama_user          = mysqli_real_escape_string($config, trim($_POST['nama_user']));
				...
				if (empty($_POST['password']) && empty($_FILES['foto']['name'])) {
                    ...
                    $query = mysqli_query($config, "UPDATE tbl_users SET username 	= '$username',
                    													nama_user 	= '$nama_user',
                    													email       = '$email',
                    													telepon     = '$telepon',
                    													hak_akses   = '$hak_akses'
                                                                  WHERE id_user 	= '$id_user'")
                                                    or die('Ada kesalahan pada query update : '.mysqli_error($config));
	...
	elseif ($_GET['act']=='on') {
		if (isset($_GET['id'])) {
			...
			// perintah query untuk mengubah data pada tabel users
            $query = mysqli_query($config, "UPDATE tbl_users SET status  = '$status'
                                                          WHERE id_user = '$id_user'")
                                            or die('Ada kesalahan pada query update status on : '.mysqli_error($config));
